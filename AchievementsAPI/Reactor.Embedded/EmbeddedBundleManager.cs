...
/// </summary>
// https://github.com/NuclearPowered/Reactor/blob/04100970b66d57a2dd96f6f4eae0383362e9a321/Reactor/Utilities/AssetBundleManager.cs
public static class EmbeddedBundleManager
{
    private static readonly StandardLogger log = LoggerFactory.GetLogger<StandardLogger>(typeof(EmbeddedBundleManager));
    ...
    private static bool TryFindFile(Assembly assembly, string fileName, [NotNullWhen(true)] out string? path)
    {
        ...
        if (pluginDirectoryPath != null)
        {
            ...
            {
                log.Debug($"Loading an asset bundle from {filePath}");
                path = filePath;
                return true;
            }
        }
    ...
    private static bool TryLoadResource(Assembly assembly, string fileName, [NotNullWhen(true)] out Il2CppStructArray<byte>? data)
    {
        ...
        {
            log.Debug($"Loading an asset bundle from {resourceName}");
